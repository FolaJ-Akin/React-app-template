...
export default function Keyboard({ randWord,swich, handleSwitch, getCurrentGuess,getTruthyArray,currentGuess}:prop):JSX.Element {
    ...
    const handleEnter = () => {
        ...
        if (ans.length>4){
            const cssTruthyArray = checkContained(letterarray.slice(attempt,attempt +5),randWord)
    ...
    return (
        <div>
            ...
            <div className="row1" >{alphabetObjectarray.slice(0,10).map((oneletter:alphabetObject)=> <div className="CharKey" key={oneletter.id} onClick={()=> handleOnletterClick(oneletter.letter)} >{oneletter.letter}</div>)}</div>
            <div className="row2" >{alphabetObjectarray.slice(10,19).map((oneletter:alphabetObject)=> <div className="CharKey" key={oneletter.id} onClick={()=> handleOnletterClick(oneletter.letter)} >{oneletter.letter}</div>)}</div>
            
            <div className="row3" > <button style={stylebackspace} onClick={handleEnter} >Enter</button> {alphabetObjectarray.slice(19,26).map((oneletter:alphabetObject)=> <div className="CharKey" key={oneletter.id} onClick={()=> handleOnletterClick(oneletter.letter)} >{oneletter.letter}</div>)}
            <button style={stylebackspace} onClick={handleBackspace}>{backspace}</button></div>
